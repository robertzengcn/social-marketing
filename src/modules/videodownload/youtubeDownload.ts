...
export class YoutubeDownload implements videoDownloadImpl {
    async downloadVideo(url:string,savePath:string,cookiesProxy?:CookiesProxy|null,proxy?:Proxy|null,execPath?:string,errorCall?:(errorMsg:string)=>void,stroutCall?:(message:string)=>void){
        if(!execPath){
