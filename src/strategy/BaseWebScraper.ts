...
import {
  OutreachScrapingStrategy,
  ScrapingOptions,
  ScrapingResult,
  ContactInfo,
  ScrapingMetadata,
  ScrapingError,
} from "./OutreachScrapingStrategy";
...
export abstract class BaseWebScraper implements OutreachScrapingStrategy {
  ...
  protected async initializeBrowser(): Promise<void> {
    ...
    // Set cookies for authenticated scraping
  ...
  protected async handleAntiBotDetection(): Promise<boolean> {
    ...
    const captchaExists = await this.page.$(
      "[data-captcha], .captcha, #captcha, .g-recaptcha"
    );
    if (captchaExists) {
      return false;
    }
  ...
  protected async simulateHumanBehavior(): Promise<void> {
    ...
    await this.page.mouse.move(Math.random() * 1000, Math.random() * 1000);
