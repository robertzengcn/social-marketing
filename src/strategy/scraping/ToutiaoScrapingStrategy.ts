...
export class ToutiaoScrapingStrategy implements ScrapingStrategy {
  private readonly selectors = {
    title: 'h1.article-title, .article-title, h1.title, .title',
    content: '.article-content, .content, .article-body, .body',
    author: '.author-name, .author, .byline',
    publishDate: '.publish-time, .time, .date',
    tags: '.tag, .tags, .category',
    codeBlocks: 'pre code, .code-block, .highlight',
    images: '.article-content img, .content img, .body img'
  };
  ...
  async extractTitle(page: Page): Promise<string> {
    try {
      ...
      console.error('Error extracting title from Toutiao:', error);
      return '';
    }
  }
  ...
  async extractContent(page: Page): Promise<string> {
    try {
      ...
      console.error('Error extracting content from Toutiao:', error);
      return '';
    }
  }
  ...
  async extractMetadata(page: Page): Promise<ArticleMetadata> {
    ...
    try {
      ...
      console.error('Error extracting metadata from Toutiao:', error);
    }
  ...
  async extractCodeBlocks(page: Page): Promise<CodeBlock[]> {
    ...
    try {
      ...
      console.error('Error extracting code blocks from Toutiao:', error);
    }
    ...
    try {
      const imageElements = await page.$$eval(this.selectors.images, (elements) => {
        return elements.map((el, index) => {
          ...
          return {
            originalUrl: img.src || '',
            fileName: `toutiao_image_${index}.jpg`,
      ...
      console.error('Error extracting images from Toutiao:', error);
    }
  ...
  // Helper method to validate Toutiao URL
  static validateToutiaoUrl(url: string): boolean {
    return url.includes('toutiao.com') || url.includes('toutiao.io');
  }
} 