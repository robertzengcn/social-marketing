...
async function performScraping(taskData: ScrapingStartData): Promise<void> {
  const { taskId, urls, options, accountId } = taskData;

  // Import scraper classes
  const {
    OutreachScrapingFactory,
  } = require("../strategy/OutreachScrapingFactory");
  ...
  // Load cookies from account if accountId is provided
  if (accountId) {
    const { AccountCookiesModule } = require("../modules/accountCookiesModule");
    const accountCookiesModule = new AccountCookiesModule();
    const cookiesEntity = await accountCookiesModule.getAccountCookies(
      accountId
    );
    if (cookiesEntity && cookiesEntity.cookies) {
      try {
        const parsedCookies = JSON.parse(cookiesEntity.cookies);
        if (Array.isArray(parsedCookies) && parsedCookies.length > 0) {
          (options as any).cookies = parsedCookies;
        }
      } catch (e) {
        console.error(`Failed to parse cookies for account ${accountId}:`, e);
      }
    }
  }
