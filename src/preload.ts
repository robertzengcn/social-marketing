...
contextBridge.exposeInMainWorld('api', {
    ...
    invoke: (channel, data) => {
      ...
      const validChannels = ['user:Login','user:checklogin','user:Signout','campaign:list','socialtask:list','socialtask:info','socialtasktype:list','tag:list','socialtask:save','socialtask:start','socialtaskrun:list','socialtaskresult:list',SOCIALACCOUNTlIST,SOCIALACCOUNTSAVE,'socialplatform:list','socialaccount:detail','socialaccount:delete','proxy:list','proxy:delete','proxy:save','proxy:detail','proxy:check','proxy:import',EXTRAMODULECHANNE_LIST,OPENDIRECTORY,VIDEODOWNLOAD,VIDEODOWNLOAD_TASK_LIST,LISTSESARCHRESUT,SEARCHSCRAPERAPI,TASKSEARCHRESULTLIST,
        ...
        EMAILMARKETINGTEMPUPDATE,EMAILMARKETINGFILTERLIST,EMAILMARKETFILTERDETAIL,EMAILMARKETFILTERUPDATE,EMAILSERVICEUPDATE,EMAILSERVICEDETAIL,EMAILSERVICELIST,EMAILSERVICEDELETE,EMAILFILTERDELETE,BUCKEMAILTASKLIST,BUCKEMAILTASKSENDLOG,VIDEODOWNLOAD_LIST,VIDEODOWN_TASK_ERROR_LOG_QUERY,VIDEODOWNLOAD_LOG_QUERY]
      if (validChannels.includes(channel)) {
